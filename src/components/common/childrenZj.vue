  ...
  <div class="mengban">
    ...
    <div class="xfv-from">
      ...
      <group title="" class="group_cont" v-if="formData.children[memberLength]">
        <x-input v-model="formData.children[memberLength].childName" title="姓名" placeholder="请填写姓名" :show-clear="true" placeholder-align="right" text-align="right" :max="15" :readonly="inputRead"></x-input>
        <custom-selector v-model="formData.children[memberLength].sex" describe="性别" :options="sexList" :isFirst="true" :disabled="seledisab"></custom-selector>
        <custom-selector v-model="formData.children[memberLength].relation" describe="关系" :options="relationList" :isFirst="true" :disabled="seledisab"></custom-selector>
        <datetime class="address_cont" v-model="formData.children[memberLength].birthdayDate" title="出生日期" :min-year=1900 :max-year="maxYear" :readonly="inputRead"></datetime>
        ...
        <custom-selector v-model="formData.children[memberLength].residenceFlag" describe="现居住地上学" :options="residenceFlagList" :isFirst="true" :disabled="seledisab"></custom-selector>
      </group>
    </div>
...
//          this.Confirm.confirmText = '请核实所有的信息的正确性，确定后将无法再更改！'
      ...
      getPeopleInfo () {
        this.$store.commit('UPDATE_LOADING', true);
        api.rebackPeopleInfo({idCard:this.formData.idNo,isEdit:this.isEdit}).then((res) => {
...
.mengban{width:100%;height:100%;background:rgba(0,0,0,0.5);position: fixed;top:0;right:0;left:0;bottom:0;z-index:99999;}
</style>
